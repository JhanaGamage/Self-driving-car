...
if(localStorage.getItem("bestBrain")){
    console.table(localStorage.getItem("bestBrain"))
...
const trafficCarWidth = 20;
const traffic = [
    new Car(
        road.getLaneCenter(2), // First is on center
        1000, // y pos
        trafficCarWidth, // width
        50, // height
        "DUMMY"
    )
];
...
function animate(time) {
    ...
    if (elapsed > fpsInterval) {
        ...
        if(allCarsDamaged){
            ...
            gameOver = true;
            bestCar = cars.find( c => c.y === Math.min( ...carsAlived.map(c=>c.y) ) ); // farest car
            //save();
            //window.location.reload();
            return;
        }
